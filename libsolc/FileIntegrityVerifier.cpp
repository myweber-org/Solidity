...
int main(int argc, char* argv[]) {
    ...
    if (FileIntegrityVerifier::verifyIntegrity(filepath, expectedHash)) {
        std::cout << "Integrity check PASSED: File matches expected SHA256 hash." << std::endl;
        return 0;
    } else {
        std::cout << "Integrity check FAILED: File does not match expected SHA256 hash." << std::endl;
        return 1;
    }
}