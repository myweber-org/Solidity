...
public:
    ...
    double verifyMultiplication(const MatrixMultiplier& reference) const {
        double error = 0.0;
        for (size_t i = 0; i < rowsA; ++i) {
            for (size_t j = 0; j < colsB; ++j) {
                error += std::abs(result[i][j] - reference.result[i][j]);
            }
        }
        return error;
    }
...
int main() {
    ...
    try {
        MatrixMultiplier multiplier1(SIZE, SIZE, SIZE, SIZE);
        MatrixMultiplier multiplier2(SIZE, SIZE, SIZE, SIZE);
        
        double startTime = omp_get_wtime();
        multiplier1.multiplySequential();
        double seqTime = omp_get_wtime() - startTime;
        
        startTime = omp_get_wtime();
        multiplier2.multiplyParallel();
        ...
        double verificationError = multiplier1.verifyMultiplication(multiplier2);
        std::cout << "Verification error between sequential and parallel: " 
                  << verificationError << "\n";
        
        if (SIZE <= 10) {
            multiplier1.displayResult();
        }
