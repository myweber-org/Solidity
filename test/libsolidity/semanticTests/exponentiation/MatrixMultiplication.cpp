...
std::vector<std::vector<double>> multiplyMatricesSerial(
    const std::vector<std::vector<double>>& A,
    const std::vector<std::vector<double>>& B) {
...
int main() {
    ...
    auto resultSerial = multiplyMatricesSerial(A, B);
    ...
    std::cout << "Speedup factor: " << serialDuration.count() / parallelDuration.count() << std::endl;
    
    bool resultsMatch = true;
    for (int i = 0; i < N && resultsMatch; ++i) {
        for (int j = 0; j < N && resultsMatch; ++j) {
            if (std::abs(resultSerial[i][j] - resultParallel[i][j]) > 1e-9) {
                resultsMatch = false;
            }
        }
    }
