...
public:
    ...
    void printInfo() const {
        ...
        if (width > 0 && height > 0) {
            ...
            std::cout << "Top-left pixel: R=" << (int)p.r << " G=" << (int)p.g << " B=" << (int)p.b << std::endl;
        }
    }
};
...
int main() {
    try {
        ...
        Image resized = Image::resize(original, 400, 300);
        std::cout << "\nResized image:" << std::endl;
        resized.printInfo();

        Image thumbnail = Image::resize(original, 100, 75);
        std::cout << "\nThumbnail image:" << std::endl;
        thumbnail.printInfo();
