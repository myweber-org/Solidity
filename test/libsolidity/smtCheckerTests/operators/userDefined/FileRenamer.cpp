...
int main(int argc, char* argv[]) {
    ...
    if (!fs::exists(targetDirectory) || !fs::is_directory(targetDirectory)) {
        std::cerr << "Error: " << targetDirectory << " is not a valid directory." << std::endl;
        return 1;
    }
    
    std::cout << "Renaming files in: " << fs::absolute(targetDirectory) << std::endl;
    std::cout << "Using base name: " << baseName << std::endl;
