    ...
    void check_for_changes() {
        ...
        for (const auto& entry : fs::directory_iterator(watch_path)) {
            ...
            if (file_map.find(file_path) == file_map.end()) {
                std::cout << "[NEW] File created: " << entry.path().filename() << std::endl;
                changes_detected = true;
            } else if (file_map[file_path] != current_time) {
                std::cout << "[MOD] File modified: " << entry.path().filename() << std::endl;
                changes_detected = true;
            }
        ...
        for (const auto& [file_path, _] : file_map) {
            if (!fs::exists(file_path)) {
                std::cout << "[DEL] File deleted: " << fs::path(file_path).filename() << std::endl;
                changes_detected = true;
