...
int main() {
    ...
    try {
        ...
        std::cout << "Sequential multiplication time: " << (endTime - startTime) << " seconds\n";
        
        startTime = omp_get_wtime();
        multiplier.multiplyParallel();
        endTime = omp_get_wtime();
        std::cout << "Parallel multiplication time: " << (endTime - startTime) << " seconds\n";
        
        double verificationError = multiplier.verifyWithSequential();
        std::cout << "Maximum verification error: " << verificationError << "\n";
        
        if (verificationError < 1e-10) {
            std::cout << "Parallel computation verified successfully.\n";
        } else {
            std::cout << "Warning: Parallel computation shows significant error.\n";
        }
