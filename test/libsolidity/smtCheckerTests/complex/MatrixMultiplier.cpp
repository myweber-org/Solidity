...
public:
    ...
    bool verifyResults(const std::vector<std::vector<double>>& seqResult) const {
        if (seqResult.size() != result.size() || (seqResult.empty() && !result.empty())) {
            return false;
        }
...
int main() {
    ...
    try {
        ...
        std::cout << "Matrix dimensions: " << ROWS_A << "x" << COLS_A << " * " 
                  << ROWS_B << "x" << COLS_B << " = " << ROWS_A << "x" << COLS_B << "\n";
