...
private:
    ...
    void compare_and_log(const std::unordered_set<std::string>& previous_files,
                         const std::unordered_set<std::string>& current_files) {
...
public:
    ...
    void start(int interval_seconds = 2) {
        running = true;
        std::cout << "Starting watcher. Checking every " << interval_seconds << " seconds." << std::endl;
        while (running) {
