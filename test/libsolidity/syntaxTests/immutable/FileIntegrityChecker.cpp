...
bool verifyFileIntegrity(const std::string& filepath, const std::string& expectedHash) {
    try {
        ...
        std::cout << "Expected SHA256: " << expectedHash << "\n";
        std::cout << "Actual SHA256:   " << actualHash << "\n";
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <filepath> <expected_sha256_hash>\n";
        return 1;
    }
    ...
    bool integrityValid = verifyFileIntegrity(filepath, expectedHash);
