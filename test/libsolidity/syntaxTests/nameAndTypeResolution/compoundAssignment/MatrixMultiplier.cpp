...
public:
    ...
    void verifyResults(const std::vector<std::vector<double>>& seqResult) const {
        double tolerance = 1e-9;
        bool isCorrect = true;
        
        for (size_t i = 0; i < rowsA && isCorrect; ++i) {
            for (size_t j = 0; j < colsB && isCorrect; ++j) {
                if (std::abs(resultMatrix[i][j] - seqResult[i][j]) > tolerance) {
                    isCorrect = false;
                }
            }
        }
        
        std::cout << "Result verification: " << (isCorrect ? "PASSED" : "FAILED") << std::endl;
    }
    ...
    void benchmarkMultiplication() {
        ...
        std::fill(resultMatrix.begin(), resultMatrix.end(), std::vector<double>(colsB, 0.0));
        ...
        std::cout << "Speedup factor: " << seqTime / parTime << "x\n";
        
        verifyResults(sequentialResult);
    }

    static void runPerformanceTest(size_t size) {
        std::cout << "\n=== Performance test for " << size << "x" << size << " matrices ===\n";
...
int main() {
    try {
        std::cout << "Matrix Multiplication Performance Benchmark\n";
        std::cout << "===========================================\n";
        
        ParallelMatrixMultiplier::runPerformanceTest(500);
        ParallelMatrixMultiplier::runPerformanceTest(1000);
        ParallelMatrixMultiplier::runPerformanceTest(1500);
