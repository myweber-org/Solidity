...
int main(int argc, char* argv[]) {
    ...
    bool isValid = DataIntegrityValidator::verifyFileIntegrity(filepath, expectedHash);
    
    if (isValid) {
        std::cout << "File integrity verified successfully." << std::endl;
        return 0;
    } else {
        std::cout << "File integrity check failed. File may be corrupted or tampered." << std::endl;
        return 1;
    }
}