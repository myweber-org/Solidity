...
public:
    explicit FileSystemWatcher(const fs::path& dir_path) : watch_path(dir_path) {
    ...
    void check_for_changes() {
        ...
        for (const auto& entry : fs::directory_iterator(watch_path)) {
            ...
            if (current_files.find(file_path) == current_files.end()) {
                std::cout << "File added: " << file_path << std::endl;
                changes_detected = true;
            } else {
                if (current_files[file_path] != current_write_time) {
        ...
        for (const auto& [deleted_file, _] : current_files) {
            std::cout << "File deleted: " << deleted_file << std::endl;
            changes_detected = true;
        }
