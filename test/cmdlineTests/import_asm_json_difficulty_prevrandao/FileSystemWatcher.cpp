...
private:
    ...
    void check_for_changes() {
        auto current_file_map = file_map;
        ...
        for (const auto& entry : fs::recursive_directory_iterator(watch_directory)) {
            ...
            } else if (current_file_map[file_path] != current_write_time) {
                std::cout << "[MODIFIED] File changed: " << file_path << std::endl;
                changes_detected = true;
                current_file_map.erase(file_path);
            } else {
                current_file_map.erase(file_path);
            }
        }

        for (const auto& [file_path, _] : current_file_map) {
