...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        ...
        std::cout << "If only filepath is provided, computes its SHA256 hash." << std::endl;
        std::cout << "If expected_hash is provided, verifies the file against it." << std::endl;
        return 1;
    }
    ...
    if (argc == 2) {
        ...
        if (isValid) {
            std::cout << "File hash matches the expected value." << std::endl;
            return 0;
        } else {
            std::cout << "File hash does NOT match the expected value!" << std::endl;
            return 1;
        }
    }
