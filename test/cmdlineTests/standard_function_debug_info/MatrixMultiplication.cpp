...
bool verify_matrices(const std::vector<std::vector<double>>& mat1,
...
int main() {
    ...
    if (verify_matrices(result_parallel, result_sequential)) {
        std::cout << "Verification: Matrices are identical within tolerance." << std::endl;
    } else {
        std::cout << "Verification: Matrices differ!" << std::endl;
    }
