...
Image resizeImage(const Image& src, int newWidth, int newHeight) {
    ...
    for (int y = 0; y < newHeight; ++y) {
        for (int x = 0; x < newWidth; ++x) {
            ...
            top.r = interpolate(dx, p00.r, p10.r);
            top.g = interpolate(dx, p00.g, p10.g);
            top.b = interpolate(dx, p00.b, p10.b);

            bottom.r = interpolate(dx, p01.r, p11.r);
            bottom.g = interpolate(dx, p01.g, p11.g);
            bottom.b = interpolate(dx, p01.b, p11.b);

            result.r = interpolate(dy, top.r, bottom.r);
            result.g = interpolate(dy, top.g, bottom.g);
            result.b = interpolate(dy, top.b, bottom.b);
