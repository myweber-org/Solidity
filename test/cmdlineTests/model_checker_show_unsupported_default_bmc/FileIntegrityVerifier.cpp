...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Verifying file: " << filepath << std::endl;
    
    if (FileIntegrityChecker::verifyFileIntegrity(filepath, expectedHash)) {
        std::cout << "SUCCESS: File integrity verified. SHA-256 matches." << std::endl;
        return 0;
    } else {
        std::cout << "FAILURE: File integrity check failed. Hashes do not match." << std::endl;
        return 1;
    }
}