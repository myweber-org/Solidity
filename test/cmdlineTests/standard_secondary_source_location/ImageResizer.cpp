...
void printImageInfo(const Image& img, const std::string& name) {
    std::cout << name << ": " << img.getWidth() << "x" << img.getHeight() << std::endl;
}
...
int main() {
    ...
    try {
        Image resized = resizeImage(original, 16, 12);
        printImageInfo(resized, "Resized");

        Image small = resizeImage(original, 4, 3);
        printImageInfo(small, "Small");
