...
void renameFilesWithTimestamp(const fs::path& directory) {
    ...
    std::string timestamp_suffix = timestamp_stream.str();

    for (const auto& entry : fs::directory_iterator(directory)) {
        if (fs::is_regular_file(entry.status())) {
            fs::path old_path = entry.path();
            std::string stem = old_path.stem().string();
            std::string extension = old_path.extension().string();
            fs::path new_path = old_path.parent_path() / (stem + timestamp_suffix + extension);
