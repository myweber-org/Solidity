...
public:
    ...
    void printTopLeft(int limit = 5) const {
        int printWidth = std::min(limit, width);
        int printHeight = std::min(limit, height);
...
int main() {
    try {
        ...
        std::cout << "\nResized image to 12x9 (top-left 5x5):" << std::endl;
        resized.printTopLeft();

        Image small = original.resize(4, 3);
        std::cout << "\nResized image to 4x3 (top-left 4x3):" << std::endl;
        small.printTopLeft(4);
