...
public:
    ...
    void displayResult(size_t maxRows = 5, size_t maxCols = 5) const {
        std::cout << "Result matrix (first " << std::min(maxRows, rowsA) 
                  << "x" << std::min(maxCols, colsB) << " elements):\n";
        
        for (size_t i = 0; i < std::min(maxRows, rowsA); ++i) {
            for (size_t j = 0; j < std::min(maxCols, colsB); ++j) {
                std::cout << result[i][j] << "\t";
            }
            std::cout << "\n";
        }
