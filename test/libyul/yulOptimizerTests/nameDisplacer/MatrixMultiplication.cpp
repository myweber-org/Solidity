...
public:
    ...
    bool verifyResults(const std::vector<std::vector<double>>& seqResult) {
        if (seqResult.size() != result.size() || seqResult[0].size() != result[0].size()) {
            return false;
        }
        ...
        for (size_t i = 0; i < result.size(); ++i) {
            for (size_t j = 0; j < result[0].size(); ++j) {
                if (std::abs(seqResult[i][j] - result[i][j]) > epsilon) {
                    return false;
                }
            }
        }
    ...
    void benchmarkMultiplication() {
        ...
        std::cout << "Sequential time: " << static_cast<double>(seqTime) / CLOCKS_PER_SEC << " seconds\n";
        std::cout << "Parallel time: " << static_cast<double>(parTime) / CLOCKS_PER_SEC << " seconds\n";
        std::cout << "Speedup: " << static_cast<double>(seqTime) / parTime << "x\n";
